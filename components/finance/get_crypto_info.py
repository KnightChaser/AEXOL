...
def check_available_market_codes(markets:str):
    ...
    availability_list_file_path = "./components/finance/data/crypto/available_market_codes.json"
    ...
    if not os.path.isfile(availability_list_file_path) or not time.time() - os.path.getmtime(availability_list_file_path) < 600:
        ...
        os.makedirs(os.path.dirname(availability_list_file_path), exist_ok = True)

        with open(availability_list_file_path, 'w+') as file:
...
def get_crypto_candle_chart(markets:str, minutes:int):
    ...
    try:
        ...
        chart_filedir  = "./components/finance/data/crypto/"       # POV : the location of bot.py which runs these code
...
# def get_crypto_info(minutes:int, markets:str):            # when you need to adjust minutes of the chart dynamically
    ...
    if market_availability == False:
        ...
        embed = discord.Embed(title = "조회 불가", color = 0XFF0000)
        embed.add_field(name = "오류!", value = "입력하신 마켓 코드는 현재 데이터 제공업체인 **업비트(Upbit)** 거래소에서 지원하지 않습니다.")
    ...
    await ctx.respond(file = candle_chart_picture, embed = embed)    
